...
### 8.7 GUI工程生成与表单构造避坑指南 (GUI Project Generation Pitfalls)

在通过 MCP 或文本工程格式（`ecode`）动态生成或重构易语言 GUI （窗口）应用程序时，必须遵循以下硬性规范，否则程序将出现编译失败、闪退或“载入启动窗口失败”等运行时报错。

#### 1. 窗口 ID 与程序集 ID 绑定规范 (防止载入启动窗口失败)
* **避坑要点**：易语言的窗口程序集（代码层）与窗体布局（UI 层）是通过唯一的 **ID 标识符** 在二进制中强绑定的。如果在 `*.form.json` 中遗漏或错误配置了以下字段，程序可以成功编译，但在运行时双击启动会抛出 `运行时出错！ 错误信息：载入启动窗口失败 <Id>, 0` 的对话框。
* **标准实现**：
  * 窗口控件的 JSON 根属性中，**必须**包含非零的 32 位整数 `"Id"`（例如 `"Id": 1375797249`）、`"UnknownAfterId"`（例如 `"UnknownAfterId": 14765456`）和 `"UnknownBeforeClass"`（例如 `"UnknownBeforeClass": 1968236`）。
  * 易语言运行时载入窗口程序集的 ID 通常是 `窗口 Id + 1`（如 `1375797250`）。如果随机或置空这些 ID，编译器将无法建立 UI 与代码的关联。推荐直接复用工作模板或原版导出的 ID 字段。

#### 2. 窗口类名与文件名强绑定规范 (默认启动窗口)
* **避坑要点**：易语言的默认启动窗口在工程中具有硬编码的关联。将主窗口的 `Class` 名称由 `"启动窗口程序集"` 更改为其他名字，或重命名其源文件名，都会导致编译器无法生成启动窗口入口。
* **标准实现**：
  * 主窗口的类名必须为 `"Class": "启动窗口程序集"`。
  * 其代码文件命名必须为 `启动窗口.form.e.txt`，且文件内声明 `.程序集 启动窗口程序集`。
  * `排序.list.txt` 必须首行指向该文件。

#### 3. 控件 `ExtensionData` 的对齐偏移量与还原
* **避坑要点**：`ExtensionData` 是易语言用于序列化控件高级属性（如标题、字体、颜色）的十六进制数据块。若在此数据块中发生哪怕 1 字节（2 个十六进制字符）的对齐错位，易语言 IDE 打开源码时便会报错 `Invalid data in the file!`（文件数据损坏）。
* **标准实现**：
  * **窗口 (DataType: "窗口")**：窗口标题属性前面的二进制前缀固定长度为 **84 字节 (168个十六进制字符)**（注意：非 77 字节/154 字符，否则会导致 7 字节的零填充缺失而错位）。后跟 32 位小端标题长度、GBK 编码标题，并以 `00000000` 结尾。
  * **按钮 (DataType: "按钮")**：按钮文本属性前面的二进制前缀固定长度为 **24 字节 (48个十六进制字符)**。后跟小端文本长度、GBK 编码文本。
  * **无损首选（推荐）**：如果不需要为控件定制复杂的字体与颜色属性，可直接将 `"ExtensionData"` 设为 `""`（空字符串）。易语言编译器在回编时会自动以默认系统属性补全，从而彻底避开二进制对齐和字节偏移量错位的问题。

#### 4. 文本编码的严格 UTF-8 BOM 规范
* **避坑要点**：`e2txt` / `ebuild` 工具在将 `ecode` 文本工程回编为 `.e` 文件时，要求 `支持库.config.json`、`*.form.json` 窗体文件以及所有 `.e.txt` 源码文件必须以 **UTF-8 with BOM** 格式保存。若写入时遗漏了 BOM 标识，回编工具可能会将其误判为 ANSI/GBK，进而引发中文字符乱码或 JSON 解析失败。
* **标准实现**：
  * 在 Python 中写入以上文本工程文件时，**必须**使用 `encoding='utf-8-sig'`。

#### 5. 静态编译与动态链接的选择策略
* **避坑要点**：如果在 `支持库.config.json` 中残留了未使用的第三方库（例如 `CnCalendar` 或 `vclbase`），但编译服务器上缺失该支持库的静态链接库（`.lib`/`.obj` 位于 `static_lib` 中），使用 `static_link: true` 进行编译会报错 `静态链接失败` (退出码 `-10`)。
* **标准实现**：
  * 如果项目包含第三方库且编译环境不完整，应在 `compile_efile` 时将 `static_link` 参数设置为 `false`（动态独立编译），确保在不依赖静态 `.lib` 的情况下生成可运行的 EXE 程序。
  * 纯核心库（`krnln`）项目推荐使用 `static_link: true`，以生成绿色便携的单文件 EXE 软件。

---

### 8.8 自动化编译与调试脚本编写规范 (Automation & Diagnostic Scripting)

在日常开发与持续构建流水线中，通常需要使用 Python 脚本自动化调用 MCP 服务的编译/回编接口，或对导出的控件元数据进行深度诊断。为保证此类脚本编写效率与健壮性，必须执行以下规范：

#### 1. HTTP SSE 与 JSON-RPC 双向握手脚本模板
* **避坑要点**：MCP 服务运行在 SSE 协议之上。如果仅发送 HTTP POST 请求而未通过 GET 方式保持 SSE 长连接流，POST 请求将无法完成数据响应。
* **标准 Python 交互模板**：
  ```python
  import os
  import json
  ...
  def listen_sse(event_container, stop_event):
      """后台线程：实时监听并缓存 SSE 推送的消息"""
          ...
          with urllib.request.urlopen(req) as response:
              for line in response:
                  ...
                  if line.startswith("event:"):
                      ...
                      data = line[5:].strip()
                      event_container.append((line, data))
      except Exception:
          pass

  def call_mcp_tool(tool_name, arguments, rpc_id, timeout_sec=20):
      """标准 RPC 工具调用封装：自动处理握手、消息推送与响应匹配"""
      event_container = []
      stop_event = threading.Event()
      t = threading.Thread(target=listen_sse, args=(event_container, stop_event))
      t.daemon = True
      t.start()
      
      # 1. 等待 SSE 流建立并推送 endpoint 路径
      post_path = None
      for _ in range(30):
          for raw, data in list(event_container):
              if "endpoint" in raw or "/message" in data:
                  post_path = data
                  break
          if post_path:
              break
      ...
      if not post_path:
          stop_event.set()
          raise RuntimeError("无法连接至 MCP SSE 终结点")
          
      # 2. 推送 JSON-RPC 请求到 message POST 路由
      rpc_req = {
          "jsonrpc": "2.0",
          "method": "tools/call",
          "params": {
      ...
      data_bytes = json.dumps(rpc_req).encode('utf-8')
      req = urllib.request.Request(f"{BASE_URL}{post_path}", data=data_bytes, headers={'Content-Type': 'application/json'})
      ...
      while time.time() - start_time < timeout_sec:
          for event, data in list(event_container):
              try:
      ...
      if not response_data:
          raise TimeoutError(f"等待工具 [{tool_name}] 响应超时")
      return response_data
  ```

#### 2. 二进制 `ExtensionData` 的十六进制对比与调试规范
* **诊断目标**：当修改控件属性导致回编出错或运行时数据损坏时，可通过编写临时的诊断脚本对 `ExtensionData` 的特定区段进行提取和十六进制值对比。
* **诊断与还原脚本模板**：
  ```python
  # 提取某十六进制字段的前缀、长度、内容和后缀，进行 GBK 解码测试
  def inspect_extension_data(hex_str, prefix_len_bytes):
      prefix_len_chars = prefix_len_bytes * 2
      prefix = hex_str[:prefix_len_chars]
      length_hex = hex_str[prefix_len_chars:prefix_len_chars + 8]
      
      # 易语言采用 32 位小端整型保存文本长度
      # 例如 "06000000" 代表长度为 6 字节 (3个汉字)
      length_val = int.from_bytes(bytes.fromhex(length_hex), byteorder='little')
      
      content_hex = hex_str[prefix_len_chars + 8 : prefix_len_chars + 8 + (length_val * 2)]
      suffix = hex_str[prefix_len_chars + 8 + (length_val * 2):]
      
      decoded_text = bytes.fromhex(content_hex).decode('gbk', errors='replace')
      
      print("--- 元数据分析 ---")
      print(f"前缀: {prefix}")
      print(f"长度值: {length_val} 字节 (Hex: {length_hex})")
      print(f"内容文本: {decoded_text}")
      print(f"后缀: {suffix}")
  ```

#### 3. 稳健的临时工程构建目录生命周期
* **避坑要点**：当编写构建脚本将文本工程重新生成 `.e` 及 `.exe` 后，若立刻调用系统级强力清除（如 `rmtree`）可能会导致后续依赖这些中间产物的分析程序崩溃。
* **构建规范**：
  * 在一键构建脚本中，**必须**保留 `temp_project`（当前构建目录）直到编译程序 `ecl.exe` 彻底退出并生成目标文件。
  * 建议使用基于 `verify_form_content.py` 的后置检验函数，对生成好的 `_启动窗口.form.json` 等文本资产做一致性断言，并在主构建任务全部完成后再清理临时目录。
